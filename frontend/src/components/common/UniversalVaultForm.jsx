    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (initialData?.asset_id) {
                // Update mode
                await axios.put(`http://localhost:5001/api/assets/${initialData.asset_id}`, {
                    category,
                    title: assetTitle,
                ...
                // Create mode
                await axios.post('http://localhost:5001/api/assets', {
                    category,
                    title: assetTitle,
                    metadata: finalMetadata
                }, {
                            ...
                            <UploadComponent
                                ...
                                onRemove={async () => {
                                    if (formData.uploadedFile?.filename) {
                                        try {
                                            const token = localStorage.getItem('stardust_token');
                                            await fetch(`http://localhost:5001/api/uploads/${formData.uploadedFile.filename}`, {
                                                method: 'DELETE',
                                                headers: { 'Authorization': `Bearer ${token}` }
                                            });
                                ...
                                onView={() => {
                                    if (formData.uploadedFile?.filename) {
                                        const token = localStorage.getItem('stardust_token');
                                        window.open(`http://localhost:5001/api/uploads/${formData.uploadedFile.filename}?token=${token}`, '_blank');
                                    }
                                }}
                            />
                        </div>
